# =============================================================================
# CHANGELOG — role_assignment.py
# Last modified: 2026-02-14 (RJS)
#
# Changes from previous version:
#
# 1. NOTE (dependency on metacognitive.py fixes):
#    metacognitive.py had several bugs that affected this module's behavior:
#      - compute_metacognitive_state_vector was ignoring the `prompts` parameter
#        (shadowed by `prompts = Prompts()`). Now fixed: the Prompts instance
#        passed from this module's pipeline is actually used.
#      - _compute_problem_importance was being called with `response` instead
#        of `original_prompt`. Now fixed: PI correctly assesses the user query.
#      - _compute_emotional_response was ignoring config weights (always using
#        default 0.1 for all ER dimensions). Now fixed: config weights are
#        passed through.
#    These fixes may change MSV values and therefore routing decisions and
#    fitness scores compared to prior runs. This is expected and correct.
#
# 2. CLEANUP (compute_routing, local import):
#    Removed `import math` from inside compute_routing() — math is now
#    imported at module level where it belongs.
#
# 3. NOTE (generate_preliminary_responses is sequential by design):
#    Although marked async, this function runs Ollama calls sequentially.
#    This is intentional: ollama.chat() is synchronous, and running multiple
#    concurrent Ollama inference calls on a single machine would not improve
#    throughput (GPU is the bottleneck). If distributed inference is added
#    later, these can be converted to asyncio.to_thread() calls.
# =============================================================================
...
NOTE ON metacognitive.py:
    metacognitive.py has been updated with an optional `model` parameter
    (default "llama3.2") so each agent can self-assess using its own model.
    All existing callers are unaffected (backward-compatible default).
    See metacognitive.py CHANGELOG for additional bug fixes (2026-02-14).
...
from system_two_model import NodeRole, NodeResponse
...
async def compute_agent_msvs(
    ...
    Each agent's MSV is computed using that agent's own model for
    self-assessment (CE, EM, CI, PI). This ensures the MSV reflects
    the generating model's metacognitive self-evaluation, not a
    cross-model peer evaluation.

    If agent_model_names is None, falls back to llama3.2 for all
    (backward-compatible with single-model operation).
    """
    import asyncio
    ...
    msvs = await asyncio.gather(*[
        compute_metacognitive_state_vector(
            ...
            model=agent_model_names[i] if agent_model_names else "llama3.2",
        )
...
async def run_role_pipeline(
    ...
    for role in DIALECTICAL_SEQUENCE:
        ...
        pipeline_model = (
            agent_model_names[agent_idx]
            if agent_model_names and agent_idx < len(agent_model_names)
            else "llama3.2"
        )

        # Build role prompt using existing prompt templates
        # (mirrors Node.get_response() from system_two_model.py lines 102-118)
        messages = [
            {
                "role": "system",
                "content": prompts.get_prompt(
                    PromptNames(f"{NodeRole(role)}_system"),
                    context={"previous_node_role": previous_role},
                ),
                "thinking": "true",
            },
            {"role": "assistant", "content": previous_response},
            {
                "role": "user",
                "content": prompts.get_prompt(
                    PromptNames(f"{NodeRole(role)}_user"),
                    context={"user_prompt": user_prompt},
                ),
            },
        ]

        logger.info(f"  Pipeline stage {role}: agent {agent_idx} ({pipeline_model})")
        print(f"[role_assignment]   Pipeline stage {role}: agent {agent_idx} ({pipeline_model})")

        # Generate response with the ASSIGNED agent's model
        ollama_response = ollama.chat(model=pipeline_model, messages=messages)
        node_response = ollama_response.message.content

        # Compute MSV using the SAME model (true self-assessment)
    ...
    if synthesizer_msv:
        ...
        if generalist_triggers:
            ...
            if gen_agent_idx is not None:
                # Use the Generalist's assigned model
                gen_model = (
                    agent_model_names[gen_agent_idx]
                    if agent_model_names and gen_agent_idx < len(agent_model_names)
                    else "llama3.2"
                )
                ...
                gen_messages = [
                    {
                        "role": "system",
                        "content": prompts.get_prompt(
                            PromptNames(f"{NodeRole.Generalist}_system"),
                            context={"previous_node_role": NodeRole.Synthesizer},
                        ),
                        "thinking": "true",
                    },
                    {"role": "assistant", "content": synthesizer_response},
                    {
                        "role": "user",
                        "content": prompts.get_prompt(
                            PromptNames(f"{NodeRole.Generalist}_user"),
                            context={"user_prompt": user_prompt},
                        ),
                    },
                ]

                logger.info(f"  Generalist triggered: agent {gen_agent_idx} ({gen_model})")
                print(f"[role_assignment]   Generalist triggered: agent {gen_agent_idx} ({gen_model})")

                gen_response = ollama.chat(model=gen_model, messages=gen_messages)
                generalist_annotation = gen_response.message.content
